...
struct Customer {
    ...
    float   base_price, initial_price, addon_price,sum_price,extra_price;
    ...
    for (i = 0; i < 1; i++) {
        ...
        switch (cust[i].choice) {
            case 1:
                ...
                cust[i].no_addon1=cust[i].no_addon1+1;
            ...
            case 2:
                ...
                cust[i].no_addon2=cust[i].no_addon2+1;
                printf("\nPrice after add-on: RM%.2f\n", cust[i].addon_price);
                cust[i].initial_price=cust[i].addon_price;
                break;
            ...
            case 3:
                ...
                cust[i].no_addon3=cust[i].no_addon3+1;
                printf("\nPrice after add-on: RM%.2f\n", cust[i].addon_price);
                cust[i].initial_price=cust[i].addon_price;
                break;
...
void get_extra_order2(struct Customer cust[]);
void get_extra_order2(struct Customer cust[1000]){
    int i;
    for(i=0;i<1;i++){
    	printf("\n\n2nd Order\n");
        printf("[1] extra order     [0] Skip to 3rd order ");
        printf("\nEnter: ");
        scanf("%d", &cust[i].choice);
        printf("\n");
        ...
        case 1: 
                ...
                FILE *fw;
               	...
               	fprintf(fw, "\t\t%s\t%s    %d\t   %d\t    %d\t\t  RM %.2f\n"
                		  ,cust[i].nasiAyam,cust[i].type, cust[i].no_addon1
                		  ,cust[i].no_addon2, cust[i].no_addon3, cust[i].initial_price);
                fclose(fw);
            cust[i].sum_price = cust[i].sum_price+cust[i].initial_price;
            break;
...
void get_extra_order3(struct Customer cust[]);
void get_extra_order3(struct Customer cust[1000]){
    int i;

    for(i=0;i<1;i++){
    	printf("\n\n3rd Order\n");
        printf("[1]extra order     [0] Skip to 4th order");
        printf("\nEnter: ");
        scanf("%d", &cust[i].choice);
        printf("\n");
...
void get_extra_order4(struct Customer cust[]);
void get_extra_order4(struct Customer cust[1000]){
    int i;
    for(i=0;i<1;i++){
    	printf("\n\n4th Order\n");
        printf("[1]extra order     [0]next customer");
        printf("\nEnter: ");
        scanf("%d", &cust[i].choice);
        printf("\n");
...
void receipt (struct Customer cust[1000], FILE *fw){
    int i;
    for (i=0; i<1; i++){ 
    cust[i].receiptno=cust[i].receiptno+1;
    fw=fopen("Receipt.txt", "a+");
    fprintf(fw, "\n%d\t\t%s\t%s    %d\t   %d\t    %d\t\t  RM %.2f\n"
...
int main()
{
    ...
    do { 
        ...
        get_extra_order2(cust);     // second extra order
        get_extra_order3(cust);     // third extra order
        get_extra_order4(cust);		// fourth extra order
        total_price (cust, ft);     // receipt total part
        
    	fflush(stdin);
        printf("\n\n\nEnter 0 to (EXIT), Else for the next customer ");
        scanf("%d", &option);
        printf("\n");
        system("cls");
    } while (option != 0);    
